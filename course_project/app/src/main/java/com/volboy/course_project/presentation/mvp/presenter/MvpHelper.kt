...
class MvpHelper<View, P : Presenter<View>>(
    ...
    fun create() {
        ...
        Log.i(App.resourceProvider.getString(R.string.log_string), "attach ${presenter.toString()}")
    }

    fun destroy(isFinishing: Boolean) {
        presenter.detachView(isFinishing)
        Log.i(App.resourceProvider.getString(R.string.log_string), "detach ${presenter.toString()}")
    }
}